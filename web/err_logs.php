...
$logtype_sql = "select LogType  from err_logs GROUP BY LogType";
$logtype_rs = getrs($logtype_sql);
$date = array(
...
if (!empty($_REQUEST['logtype']) and ($_REQUEST['logtype'] !== "All"))
{
	$logtype = $_REQUEST['logtype'];
	$type_sql = " and LogType = '$logtype' ";
}
else
{
	$logtype = "All";
...
if ( !empty($_GET ['delete']) and !empty($_GET ['delid']) and $s_u_level == 1)
{
	...
	$del_sql = "delete from err_logs where id in ".$sql_del_id;
	do_sql($del_sql);
	save_do($s_u_id,$s_u_level,"用户".$s_u_name." 删除".$del_num."条系统日志操作记录成功!");
...
else
{
	...
	$sql_errlogs= "select LogType,LogContent,AddTime,id from err_logs where  AddTime >= '$starttime' and AddTime <= '$endtime'  $type_sql  order by id desc";
	$sql_errlogs_num = "select count(id) from err_logs where  AddTime >= '$starttime' and AddTime <= '$endtime'  $type_sql ";


$pagesize = $_SESSION['err_logs_pagesize'];
$num_all = getrs($sql_errlogs_num);
$num_all = $num_all[0][0];
...
$sql_errlogs = $sql_errlogs." limit ".$offset.",".$pagesize;
$errlogs = getrs( $sql_errlogs );
...
<title><?php echo $TITLE_NAME."-";?>系统日志</title>
    ...
    <td><table width="100%" height="5" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="td_no_down">
          <tr  >
            <td height="33" background="image/tab_bg1.gif"  >&nbsp;&nbsp;&nbsp;&nbsp;<span class="title1">&nbsp;系统日志</span></td>
          </tr>
          <tr>
            <td ><table width="98%" height="32" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
                <td><?php 
	echo "&nbsp;类型:&nbsp;";
	echo "<select name='logtype' class='anniu' id='logtype' onChange='this.form.submit()'>";
	echo "<option value=";
	echo  "All";
	echo  ">";
	echo  "All";; 
	echo "</option>";		
for( $i=0;$i<count($logtype_rs);$i++ ) 
 {

 	if ($logtype_rs[$i][0] == $logtype )
	{
...
?>
                  ...
                  <input name="date" type="hidden" id="date" value="<?php echo $getdate; ?>"></td>
                <td><div align="center">
...
?></td>
                <td><div>
                    <div align="center">
                      <a href="err_logs.php?user=<?php echo $user;?>&logtype=<?php echo $logtype;?>&date=today" id="today"  >今日</a>&nbsp; <a href="err_logs.php?user=<?php echo $user;?>&logtype=<?php echo $logtype;?>&date=yesterday" id="yesterday">昨日</a>&nbsp;&nbsp;<a href="err_logs.php?user=<?php echo $user;?>&logtype=<?php echo $logtype;?>&date=lastweek" id="lastweek" class="" >最近7天</a>&nbsp;&nbsp;<a href="err_logs.php?user=<?php echo $user;?>&logtype=<?php echo $logtype;?>&date=last2week" id="last2week">最近15天</a> <a href="err_logs.php?user=<?php echo $user;?>&logtype=<?php echo $logtype;?>&date=all" id="all">所有</a></div>
                </div></td>
  ...
  <tr>
    <td><div align="center">
      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC" class="Ptable" >
        ...
        <?php  
for( $i=0;$i<count($errlogs);$i++ )
{ 
	echo  "<tr>";	
	 if ($s_u_level == 1)	{?>
        <td height=18 class=Ptable><input name="delid[]" type="checkbox" class="radio" id="delid[]" value="<?php echo $errlogs[$i][3];?>"></td>
              <?php }
	echo "<td height=18 class=Ptable>";	
	echo $i+1+$pagesize*($page-1);
	echo "</td>";
	for( $j=0;$j<count($errlogs[0])/2-1;$j++ ) 
	{
		echo "<td height=18 class=Ptable>";	
		echo $errlogs[$i][$j];
		echo "</td>";
...
?>
      ...
      <table width="98%" height="32" border="0" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td><div align="right">
            <?php
$page_url = basename(__FILE__)."?user=".$user."&stime=".$starttime."&etime=".$endtime."&logtype=".$logtype."&page=";
