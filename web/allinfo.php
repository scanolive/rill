...
$url = "allinfo.php?ip=".$ip."&group=".$group."&t=".rand();
$result = socket_connect($socket, $_SESSION['python_server_ip'], $_SESSION['python_server_port']);

$cmd = "OLIVE_PHP"."@!@".$ip."@!@OLIVE_CLIENT_SHEEL#_#_#runall@!@OLIVE_EOC";
socket_write($socket, $cmd, strlen($cmd));
$data = "";
	while(true)
{
	$buf = socket_read($socket,1024,PHP_NORMAL_READ);
	if((!$buf) or($buf == "OLIVE_EOS"))
	{ 
		break; 
	}
	$data.=$buf;
	if(trim($buf) == "OLIVE_EOS"){break 2;}
    if(substr($buf, -2) == "\n"){    break;}
...
// print $data;
	...
	$sql="select Data_txt from info_day Left Join ipinfo ON ipinfo.id=info_day.Ipid where ip='$ip' and AddTime > '$date_today' order by info_day.id  desc  limit 1";
	$data=getrs($sql);
	$data = $data[0][0];
...
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="180" valign="top" class="td_clo_blue">&nbsp;</td>
    <td width="10"></td>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="td_clo_blue">
        ...
        <tr>
          <td><table width="96%" border="1" align="center" class="bgah">
            <tr>
              <td><table width="100%" border="1" align="center" class="bgah">
                  <tr>
